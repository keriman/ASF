...
$return_page = ($_SESSION["tipo"] == "almacen") ? "../../almacen/index.php" : "../index.php";

// Verificar si existe el parámetro GET
if (isset($_GET['gg']) && $_GET['gg'] !== '') {
    $subs_folio = htmlentities($_GET['gg']);
    $folio_editar = $_GET['gg'];
    
    // Consulta para obtener el status actual
    $consulta_status = "SELECT status FROM adm_pedidos WHERE folio = '$folio_editar'";
    $resultado_status = mysql_query($consulta_status) or die("Error al consultar status: " . mysql_error());
    $row_status = mysql_fetch_array($resultado_status);
    $status_original = $row_status['status'];
						...
						<div class="row">
							<div class="input-group mb-3">
								...
								<div class="col-xs-12 col-sm-4 col-md-4 col-lg-2">
									...
									<input type="date" name="fs" id="fs" class="form-control" value="<?=$salida;?>"
									<?php if($status>20){echo " readonly";} ?>>
								</div>
							</div>
						</div>
