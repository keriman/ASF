...
$default_status  = ($_SESSION["tipo"] == "almacen") ? 30 : 10; // Status según el tipo de usuario
$observaciones = $_POST['cc'];


$check_pedido = mysql_query("SELECT status FROM adm_pedidos WHERE folio = '".$folio."' LIMIT 1");
if (mysql_num_rows($check_pedido) > 0) {
    $row_status = mysql_fetch_array($check_pedido);
    $status_actual = $row_status['status'];
    
    // Si el pedido tiene status 30 (en almacén), mantenerlo
    if ($status_actual == 30) {
        $status = 30;
    } else {
...
if (mysql_num_rows($resultado)>0) {
    ...
    $sql = "UPDATE adm_pedidos SET vendedor='$vendedor', destino='$destino', cliente='$cliente', ruta='$ruta', fecha_salida='$fecha_salida', status='$status' WHERE folio='$folio'";
} else {
