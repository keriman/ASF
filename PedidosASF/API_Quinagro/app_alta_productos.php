    ...
    if (isset($pdo) && $pdo instanceof PDO) {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($cantidad > 0) {
                ...
                $sql = "UPDATE prc_pedidos SET cantidad = :cantidad, procesado = 0, usuario = 'Quinagro' 
                ...
                $sql = "UPDATE prc_pedidos SET procesado = -1, usuario = 'Quinagro' 
                        WHERE folio = :folio AND producto = :producto AND presentacion = :presentacion";
            ...
            $sql = "INSERT INTO prc_pedidos (folio, producto, presentacion, cantidad, usuario) 
                    VALUES (:folio, :producto, :presentacion, :cantidad, 'Quinagro')";
        ...
        if ($modificado && !empty($accion)) {
            ...
            if ($check_table && $check_table->rowCount() > 0) {
                $sql_obs = "INSERT INTO obs_pedidos (folio, observaciones, usuario) VALUES (:folio, :observaciones, 'Quinagro')";
        ...
        if (!empty($comentarios)) {
            ...
            $check_table = $pdo->query("SHOW TABLES LIKE 'obs_pedidos'");
            if ($check_table && $check_table->rowCount() > 0) {
                $sql_obs_comentario = "INSERT INTO obs_pedidos (folio, observaciones, usuario) VALUES (:folio, :observaciones, 'Quinagro')";
        ...
        if ($result_check->num_rows > 0) {
            ...
            if ($cantidad > 0) {
                $sql = "UPDATE prc_pedidos SET cantidad='$cantidad_safe', procesado=0, usuario='Quinagro' 
            ...
            $sql = "INSERT INTO prc_pedidos (folio, producto, presentacion, cantidad, usuario) 
                    VALUES ('$folio_safe', '$producto_safe', '$presentacion_safe', '$cantidad_safe', 'Quinagro')";
        ...
        if ($modificado && !empty($accion)) {
            ...
            if ($check_table && $check_table->num_rows > 0) {
                $sql_obs = "INSERT INTO obs_pedidos (folio, observaciones, usuario) VALUES ('$folio_safe', '$observacion_safe', 'Quinagro')";
                file_put_contents($log_file, "SQL de observación (acción): $sql_obs\n", FILE_APPEND);
        ...
        if (!empty($comentarios)) {
            ...
            $check_table = $mysqli->query("SHOW TABLES LIKE 'obs_pedidos'");
            if ($check_table && $check_table->num_rows > 0) {
                $sql_obs_comentario = "INSERT INTO obs_pedidos (folio, observaciones, usuario) VALUES ('$folio_safe', '$observacion_comentario_safe', 'Quinagro')";
                file_put_contents($log_file, "SQL de observación (comentario): $sql_obs_comentario\n", FILE_APPEND);
