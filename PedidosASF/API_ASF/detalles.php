...
try {
    ...
    $debug_info[] = "Variables de conexión disponibles:";
    $debug_info[] = "- host: " . (isset($host) ? $host : 'no disponible');
    $debug_info[] = "- username: " . (isset($username) ? 'disponible' : 'no disponible');
    $debug_info[] = "- dbname: " . (isset($dbname) ? $dbname : 'no disponible');
    $debug_info[] = "- pdo: " . (isset($pdo) ? 'disponible' : 'no disponible');
    ...
    // Decidir qué método de conexión usar
    if (isset($pdo) && $pdo instanceof PDO) {
        $debug_info[] = "Usando conexión PDO";
        
        // Test específico para el producto problemático utilizando PDO
        $test_sql = "SELECT * FROM prc_pedidos WHERE producto = 'tecnogreen 1 kg' AND folio = 4035017";
        $test_stmt = $pdo->query($test_sql);
        if (!$test_stmt) {
            $debug_info[] = "Error consulta de producto específico: " . implode(", ", $pdo->errorInfo());
        } else {
            $prod_test = $test_stmt->fetch(PDO::FETCH_ASSOC);
            if ($prod_test) {
        ...
        $stmt_pedido->bindParam(':folio', $folio, PDO::PARAM_INT);
        $stmt_pedido->execute();
        ...
        $stmt_observaciones->bindParam(':folio', $folio, PDO::PARAM_INT);
        ...
        $debug_info[] = "Usando conexión MySQLi";
        ...
        $debug_info[] = "Conectando con: host=$host_var, user=$user_var, db=$db_var";
        
        // Establecer la conexión a la base de datos
        $conn = mysqli_connect($host_var, $user_var, $pass_var, $db_var);
        if (!$conn) {
