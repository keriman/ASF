...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    try {
        // Adaptado a la tabla 'usuarios' existente
        $stmt = $db->prepare('SELECT * FROM usuarios WHERE user = :username');
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // Determinar el rol basado en el nivel de usuario
...
// Función para verificar si el usuario es administrador (level=1)
function isAdmin() {
